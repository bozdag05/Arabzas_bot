...
from keyboards.inline import again_write_arab_ikb
...
@dp.callback_query_handler(text_contains='write_arab:start')
async def start_card(call: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await state_1.write_state_arab.set()
...
@dp.message_handler(state=state_1.write_state_arab)
async def write_arab(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"Вы завершили повторение урока\n\n"
                             f"повторить ещё раз ?", reply_markup=again_write_arab_ikb)
