...
async def start_card(call: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await call.message.answer(f'{word.word}')
        await my_state.my_write_state_arab.set()
...
@dp.message_handler(state=my_state.my_write_state_arab)
async def write_arab(message: Message, state: FSMContext):
    ...
    try:
        ...
        elif word.translation.lower() == answer.lower():
        ...
        await message.answer(f"Вы завершили повторение урока\n\n"
                             f"повторить ещё раз ?", reply_markup=again_mywrite_arab_ikb)
