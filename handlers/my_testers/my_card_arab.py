...
from keyboards.inline import again_mycard_arab_ikb
from loader import dp, bot
from aiogram.types import InlineKeyboardButton, InlineKeyboardMarkup
from utils.db_api import my_word_commands as my_commands


@dp.callback_query_handler(text_contains='card_my_arab:start')
    ...
    try:
        ...
        ikb_1 = InlineKeyboardMarkup(row_width=1,
                                 inline_keyboard=[[InlineKeyboardButton(text=word.word,
                                                                        callback_data=f'card_my_arab:{word.translation}:{word.word_id}')]])
        await call.message.answer('_________Карточка_________', reply_markup=ikb_1)
    except Exception:
        await call.message.answer('Что-то не так')


@dp.callback_query_handler(text_contains='card_my_arab')
async def tester(call: CallbackQuery):
    ...
    try:
        ...
        await call.message.answer(f"Вы завершили повторение урока\n\n"
                                  f"повторить ещё раз ?", reply_markup=again_mycard_arab_ikb)
